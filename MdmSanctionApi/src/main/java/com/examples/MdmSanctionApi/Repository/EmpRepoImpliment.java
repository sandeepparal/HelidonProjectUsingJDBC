...
@ApplicationScoped
public class EmpRepoImpliment  implements EmpRepo{
...
//			query.setParameter("emp_name_en", request.getEmp_name_en());
	...
	/// Multiple Search By Id
	
	public List<EmployeeTypeResponse> getEmpMult(EmployeeTypeRequest request) throws IFMSException{
		String sql="select e.emp_name_en,e.emp_code,e.father_name,e.mother_name,e.date_of_birth,st.status_id from employee_type e "
				+ "left join mst_emp_status st on st.status_id=e.status_id  where e.emp_type_id in (:data)";
		List<EmployeeTypeResponse>	responses = new ArrayList<>();
		
		try {
			Query query=entityManager.createNativeQuery(sql);		
			query.setParameter("data", request.getEmp_type_id());
	...
	@Override
	public List<EmployeeTypeResponse> getEmpMultNa(EmployeeTypeRequest request) throws IFMSException {

		 List<EmployeeTypeResponse> responses=new ArrayList<>();
//		try {
			Query query =entityManager.createNativeQuery("select e.emp_name_en,e.emp_code,e.father_name,e.mother_name,e.date_of_birth,st.status_id from employee_type e "
								+"left join mst_emp_status st on st.status_id=e.status_id  where e.emp_name_en in (:data)");
			query.setParameter("data",request.getEmp_name_en());
